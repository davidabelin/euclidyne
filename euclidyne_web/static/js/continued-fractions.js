...
if (model) {
  ...
  const range = document.getElementById("cfRange");
  const prev = document.getElementById("cfPrev");
  const next = document.getElementById("cfNext");
  const toggle = document.getElementById("cfToggleArcs");
  const stageLabel = document.getElementById("cfStageLabel");
  const stageInfo = document.getElementById("cfStageInfo");
  const svg = document.getElementById("cfGeometry");
  const tableRows = Array.from(document.querySelectorAll("#cfConvergentTable tbody tr"));
  ...
  function renderState() {
    ...
    tableRows.forEach((row) => {
      row.classList.toggle("active", Number(row.dataset.stage) === currentStage);
    });
    prev.disabled = currentStage === 1;
    next.disabled = currentStage === stages.length;
    range.value = String(currentStage);
    toggle.textContent = showArcs ? "Hide Arcs" : "Show Arcs";
    renderGeometry();
  }
